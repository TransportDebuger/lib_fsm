...
# --------------------------------------
PROJECTNAME = s21_fsm
# Настройка типа сборки библиотеки (может быть переопределено параметром make LIBTYPE=<static, dynamic>)
LIBTYPE ?= static
...
# --------------------------------------
# Каталог содержащий исходные тексты
SRCDIR = $(CURDIR)/src
INCDIR = $(CURDIR)/include
# Каталог объектных файлов
OBJDIR= $(CURDIR)/obj
# Каталог исходных кодов тестов
TESTSRCDIR ?= $(CURDIR)/tests
# Исходный код тестов
TESTSOURCES := $(wildcard $(TESTSRCDIR)/*.c)
# Каталог сборки тестов
TESTBUILDDIR ?= $(CURDIR)/tests/build
# Имя исполняемого файла тестов
TESTEXEC ?= test_runner
# Каталог сборки библиотеки
BUILDDIR := $(CURDIR)/build
# Каталог генерации документации (может былть переопределени make DOCDIR=/some/dir)
DOCDIR ?= $(CURDIR)/docbuild/
# Каталог файлов покрытия
COVERAGEDIR  := $(TESTSRCDIR)/coverage
# Каталог установки (может быть переопределен параметрами: make INSTALLDIR=/some/dir)
INSTALLDIR ?= ../build/lib
# Каталог установки бинарных файлов
...
# --------------------------------------
CC := gcc
# Настройка флагов компиляции для production сборки
...
COVERAGE_CFLAGS := $(CFLAGS) --coverage -g -DTEST_INTERNAL

# Настройка флагов компиляции для тестирования
ifeq ($(UNAME_S), Linux)
...
install: build
	...
	@mkdir -p $(INSTALLINCDIR) $(BINDIR)
	@cp $(INCLUDES) $(INSTALLINCDIR)/
...
$(OBJDIR_COV)/%.o: $(SRCDIR)/%.c $(INCLUDES)
	@mkdir -p $(dir $@)
	$(CC) $(COVERAGE_CFLAGS) -I$(INCDIR) -I$(TESTSRCDIR) -c $< -o $@
...
unit-test-build: check-dep-unit $(COVERAGE_LIB)
	...
	@if [ -z "$(TESTSOURCES)" ]; then \
		$(call print-error, No test sources found in $(TESTSRCDIR)); exit 1; \
	fi
	@$(CC) $(COVERAGE_CFLAGS) -I$(INCDIR) -I$(TESTSRCDIR) -L$(BUILDDIR) \
